    ...
    Scaffold(
        ...
        Column(
            ...
            SettingsSection(
                ...
                Text("Cache: ${state.cacheSize} bytes", style = MaterialTheme.typography.bodySmall)
                ...
                if (state.runtimeMissingOptionalModules.isNotEmpty()) {
                    Text(
                        "Optional helpers not bundled: ${state.runtimeMissingOptionalModules.joinToString(", ")}",
                        style = MaterialTheme.typography.bodySmall,
            ...
            SettingsSection(
                ...
                Text(
                    if (state.cookiesImported) "cookies.txt ready (${state.cookiesSize} bytes)"
                    else "No Gallery DL cookies imported",
                    style = MaterialTheme.typography.bodySmall,
                )
                Spacer(Modifier.height(8.dp))
                Row(
            ...
            SettingsSection(
                ...
                title = "Expert Config",
                description = "Raw gallery-dl JSON. Normal downloads do not require editing this file.",
            ) {
                OutlinedTextField(
                    ...
                    supportingText = { Text(if (state.configValid) "Valid JSON" else "Invalid JSON") },
                )
                Spacer(Modifier.height(8.dp))
                Row(
                    modifier = Modifier.fillMaxWidth(),
